...
def k_fold(net, k, X_train, y_train, num_epochs, learning_rate, weight_decay, batch_size):
    ...
    for i in range(k):
        ...
        # 训练num_epochs
        net = train(net, X_train, y_train, X_valid, y_valid,
...
def optimize_credibility_factors(mlp_probs, cnn_probs, true_labels):
    num_classes = mlp_probs.shape[1]
    initial_params = np.ones(2 * num_classes) / 2
    bounds = [(0, 1) for _ in range(2 * num_classes)]
    constraints = ({'type': 'eq', 'fun': constraint})
    result = minimize(objective_function, initial_params,
                      args=(mlp_probs, cnn_probs, true_labels),
                      constraints=constraints, bounds=bounds)
    optimal_params = result.x
    optimal_ai = optimal_params[:num_classes]
    optimal_bi = optimal_params[num_classes:]
    return optimal_ai, optimal_bi
...
if __name__ == "__main__":
    ...
    for dB in dB_list:
        ...
        num_outputs = 11
        units_hidden = 64
        k, num_epochs_mlp, lr, batch_size, weight_decay = 5, 50, 0.01, 256, 0
        net_mlp = get_net(num_inputs, units_hidden, num_outputs, 0.5)
        net_mlp = k_fold(net_mlp, k, train_features_mlp, train_labels_mlp,
        ...
        num_epochs_cnn = 50
        model_cnn = train_model(model_cnn, train_loader, criterion, optimizer, num_epochs_cnn)
        ...
        # 优化可信度因子
        optimal_ai, optimal_bi = optimize_credibility_factors(mlp_train_probs, cnn_train_probs, train_labels_mlp.numpy())
    ...
    metrics_df.to_csv('../output_data4/my_idea_metrics.csv', index=False)