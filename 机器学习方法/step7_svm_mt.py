...
if __name__ == '__main__':
    ...
    for select_SNR in SNR_list:
        ...
        clf2=svm.SVC(C=1,kernel='rbf',probability=True)
        clf2.fit(X_train,y_train)

        y_pred = clf2.predict(X_test.values)
        y_pred_proba = clf2.predict_proba(X_test.values)

        y_hat = clf2.predict(X_train.values)
    ...
    #metrics_df.to_csv('../output_data3/SVM.csv', index=False)
