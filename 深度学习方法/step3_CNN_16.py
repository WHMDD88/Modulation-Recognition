...
def get_data():
    with open('../output_data3/abs_cyclic_spectrum16.pickle','rb') as f:
...
class CNNClassifier(nn.Module):
    ...
    def _calculate_fc_input_dim(self):
        # 计算全连接层的输入维度
        x = torch.randn(1, 1, 21, 16)  # 修改为 (21, 16)
        x = self.conv_block1(x)
        x = self.conv_block2(x)
...
if __name__ == "__main__":
    ...
    metrics_df.to_csv('../output_data4/epoch500_CNN16.csv', index=False)
    all_losses_df.to_csv('../output_data4/epoch500_CNN_losses16.csv', index=False)