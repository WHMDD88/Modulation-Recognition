...
def calculate_y_max(sample):
    ...
    acn = an - 1
    # 计算FFT
    fft_result = np.fft.fft(acn)
    # 计算功率密度
    power_density = np.abs(fft_result) ** 2
    # 计算γ_max
    y_max = np.max(power_density) / Ns
    return [y_max]
...
def get_data(data_dict):
    ...
    for key, samples in data_dict.items():
        ...
        for sample in samples:
            y_max = calculate_y_max(sample)
            sample_results.append(y_max)
...
if __name__ == '__main__':
    ...
    """result_data=get_data(normalized_dict)
    print(result_data[('QPSK',2)])
    print(len(result_data.keys()))

    with open ('../output_data5/power_density.pickle','wb') as f:
        pickle.dump(result_data,f)"""

    with open('../output_data/power_density.pickle','rb') as f:
